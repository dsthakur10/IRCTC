...
int client(int sock){
	...
	if (choice == 1){					// Log in
		...
		strcpy(password,getpass("\tPlease enter password:: "));
		...
		if(valid){
			printf("\tCredentials matched. Login successfully\n");
			...
			printf("\tInvalid Credentials. Login Failed : Incorrect password or login id\n");
			return 1;
		}
	}
